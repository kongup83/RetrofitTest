...
import com.kongup.common.server.coroutines.RetrofitCoroutineTask
...
class MainActivity : AppCompatActivity(), SwipeRefreshLayout.OnRefreshListener, RetrofitCoroutineTask.RetrofitCallback
{
    ...
    {
        val retrofitCoroutineTask = RetrofitCoroutineTask(this@MainActivity, WeatherAPIs.GET_LOCATION_SEARCH )

        val call = mWeatherAPIs.getLocationSearch( strSearch )

        retrofitCoroutineTask.setRetrofitCallback(this)
        retrofitCoroutineTask.execute<GetLocationSearch>(call)
    }


    private fun getLocation( nWoeid: Int)
    {
        val retrofitAsyncTask = RetrofitCoroutineTask(this@MainActivity, WeatherAPIs.GET_LOCATION )

        val call = mWeatherAPIs.getLocation( nWoeid )

        retrofitAsyncTask.setRetrofitCallback(this)
        retrofitAsyncTask.execute<GetLocation>(call)
    }
